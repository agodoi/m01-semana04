...
class Scene1 extends Phaser.Scene {
  ...
  preload() {
    // Carregando imagens e outros recursos
  }

  create() {
    this.add.text(100, 100, 'Insira seu nome:', { fill: '#ffffff' });

    const input = this.add.dom(300, 200, 'input', 'background-color: #ffffff; width: 200px; height: 30px; font: 16px Arial');

    const button = this.add.dom(300, 250, 'button', 'width: 208px; height: 36px; font: 16px Arial; background-color: #4CAF50; color: #ffffff').setText('Iniciar');

    button.addListener('click');

    button.on('click', () => {
      const playerName = input.node.value;
      this.scene.start('Scene2', { playerName });
    });
  }
}
